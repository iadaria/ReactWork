...
export function fetchEventsFromFirestore(filter, startDate, limit = 2, lastDocSnapshot = null) {
...
export async function setMainPhoto(photo) {
    ...
    try {
        ...
        await batch.commit(); //if error all rollback
...
export async function followUser(profile) {
    ...
    try {
        batch.set(
            db.collection('following').doc(user.uid).collection('userFollowing').doc(profile.id), 
            {
                displayName: profile.displayName,
